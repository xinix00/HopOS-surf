...
func Open(addr, name string, w, h int, logf func(string, ...any)) (*Window, error) {
	...
	// Ook de éérste verbinding is zelfherstellend: op een verse boot zaait
	// init[] launcher en display tegelijk en verliest de launcher de race
	// (dial op een display die nog downloadt) — exit'en betekende dan een
	// hele re-plaatsing (2x launcher.elf per boot, gemeten 19-07). Wachten
	// tot de display er is, is het juiste GUI-gedrag.
	for try := 0; ; try++ {
		err := win.connect()
		if err == nil {
			break
		...
		if try%5 == 0 {
			logf("window: display %s nog niet bereikbaar (%v) — blijven proberen", addr, err)
		}
...
func (win *Window) Close() error {
	...
	// Korte deadline: dit loopt in het 250ms-exitbudget van applib.OnExit —
	// een verstopte verbinding mag het afscheid niet ophouden.
